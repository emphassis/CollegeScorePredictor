...
{
    public class ActionConferenceBetService
    {
        private IDbContextFactory<AppDbContext> factory;
        public ActionConferenceBetService(IDbContextFactory<AppDbContext> _factory)
        {
            factory = _factory;
        }
        ...
        public async Task CreateBets(int week, int year)
        {
            ...
            using (var db = await factory.CreateDbContextAsync())
            {
                ...
                var teamGames = await (from g in db.GamePredictionConference
                ...
                foreach (var odd in odds.games)
                {
                    ...
                    var gameScores = await (from h in db.ScorePredictionConference
                                            where h.GamePredictionConferenceId == gamePrediction.GamePredictionConferenceId
                    ...
                    if (!noSpread)
                    {
                        ...
                        //if (betHomeSpread)
                        //{
                        //    var didLoseBeforeAway = await (from bh in db.BetHistoryConference
                        //                                   join b in db.BetConference on bh.BetConferenceId equals b.BetConferenceId
                        ...
                        //    var didLoseBeforeHome = await (from bh in db.BetHistoryConference
                        //                                   join b in db.BetConference on bh.BetConferenceId equals b.BetConferenceId
                        //                                   where bh.Year == year && bh.Week > 2 &&
                        //                                   bh.Won == false
                        //                                   && b.BetType == (int)BetTypes.HomeSpread
                        ...
                        //if (betOver)
                        //{
                        //    var didLoseBefore = await (from bh in db.BetHistoryConference
                        //                               join b in db.BetConference on bh.BetConferenceId equals b.BetConferenceId
                        //                               where bh.Year == year && bh.Week > 2 &&
                        //                               bh.Won == false
                    ...
                    if (betHomeSpread)
                    {
                        ...
                        //betGame = GetBetBool(betGame, BetTypes.HomeSpread, publicBetSpreadAway, publicBetSpreadHome, publicBetOver, publicBetUnder, publicSpreadHomeCount);

                        var bet = new BetConferenceDbo
                        {
                        ...
                        await db.BetConference.AddAsync(bet);
                    }
                    if (betAwaySpread)
                    {
                        //specific to this model
                        //odds suck 10-10 3-4 no winning combo
                        betGame = false;
                        //betGame = GetBetBool(betGame, BetTypes.AwaySpread, publicBetSpreadAway, publicBetSpreadHome, publicBetOver, publicBetUnder, publicSpreadAwayCount);
                    ...
                    if (betOver)
                    {
                        ...
                        //betGame = GetBetBool(betGame, BetTypes.Over, publicBetSpreadAway, publicBetSpreadHome, publicBetOver, publicBetUnder, publicBetOverCount);
                    ...
                    if (betUnder)
                    {
                        ...
                        //betGame = GetBetBool(betGame, BetTypes.Under, publicBetSpreadAway, publicBetSpreadHome, publicBetOver, publicBetUnder, publicBetUnderCount);
        ...
        private bool GetBetBool(bool bet, BetTypes betType, bool awaySpread, bool homeSpread, bool over, bool under, double odd)
        {
            if (!bet)
            {
                return false;
