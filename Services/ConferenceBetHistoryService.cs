...
{
    public class ConferenceBetHistoryService
    {
        private IDbContextFactory<AppDbContext> factory;
        public ConferenceBetHistoryService(IDbContextFactory<AppDbContext> _factory)
        {
            factory = _factory;
        }
        ...
        public async Task CreateHistory(int week, int year)
        {
            ...
            {
                var bets = await (from b in db.BetConference
                                  where b.Week == week
                                  && b.Year == year
                ...
                foreach (var bet in bets)
                {
                    ...
                    var betHistory = new BetHistoryConferenceDbo
                    {
                        BetConferenceId = bet.BetConferenceId,
                        Week = week,
                        Year = year,
                        Won = wonBet
                    };
                    await db.BetHistoryConference.AddAsync(betHistory);
        ...
        public async Task<GetBetHistoryResponseModel> GetHistory(int week, int year, bool onlyWon = false, bool onlyQualified = true)
        {
            using (var db = await factory.CreateDbContextAsync())
            {
                ...
                var betHistory = db.BetHistoryConference.Where(x => x.Week == week && x.Year == year);
