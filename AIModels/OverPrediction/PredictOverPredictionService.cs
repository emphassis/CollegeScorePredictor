...
{
    public class PredictOverPredictionService
    {
        private IDbContextFactory<AppDbContext> factory;
        public PredictOverPredictionService(IDbContextFactory<AppDbContext> _factory)
        {
            factory = _factory;
        }

        public async Task<PredictGameResponseModel> PredictGame(PredictGameRequestModel requestModel)
        {
        ...
        public async Task<List<PredictGameResponseModel>> GetWeekPredictions(int week, int year)
        {
            using (var db = await factory.CreateDbContextAsync())
            {
                var games = await (from g in db.GamePrediction
                ...
                {
                    var scores = await (from s in db.ScorePrediction
                                        where s.GamePredictionId == game.GamePredictionId
